...
<template>
    ...
    <section class="relative min-h-screen overflow-hidden bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-white">
        ...
        <div class="absolute inset-0">
            ...
            <!-- Light overlays -->
            <div class="absolute inset-0 bg-linear-to-b from-white/35 via-slate-100/50 to-slate-25 dark:hidden"></div>
            <div class="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(13,44,153,0.18),transparent_45%)] dark:hidden"></div>

            <!-- Dark overlays -->
            <div class="absolute inset-0 hidden bg-linear-to-b from-black/35 via-slate-950/75 to-slate-950 dark:block"></div>
            <div class="absolute inset-0 hidden bg-[radial-gradient(circle_at_top,rgba(47,69,255,0.28),transparent_45%)] dark:block"></div>
        </div>
        ...
        <div class="relative z-10 mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-6 sm:px-6 lg:px-8">
            <div class="mt-6">
                <Link
                    href="/"
                    class="inline-flex items-center space-x-3 rounded-full border border-slate-300/80 bg-white/70 px-4 py-1.5 text-xs font-semibold tracking-[0.2em] text-slate-800 uppercase backdrop-blur-md transition hover:bg-white
                    dark:border-white/20 dark:bg-white/10 dark:text-white/85 dark:hover:bg-white/20 font-heading"
                >
                    <BookOpen />
                    <span>{{ $t('History') }}</span>
                </Link>
            ...
            <div id="our-mission" class="pb-10 sm:pb-14">
                <div class="max-w-4xl rounded-2xl border border-slate-300/70 bg-white/75 p-6 backdrop-blur-sm sm:p-8 dark:border-white/12 dark:bg-black/25">
                    <h2 class="text-2xl font-black tracking-tight sm:text-3xl">Nuestra misión</h2>
