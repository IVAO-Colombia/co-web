...
/**
 * The time a slot is chronologically ordered by: an arrival's ETA (when it
 * lands at the hub), a departure's EOBT (when it leaves). Falls back to the
 * other time if the primary one is missing, so every slot still sorts.
 */
function operationalTime(slot: PilotSlot): string {
    return slot.category === PilotSlotCategory.ARRIVAL
...
const pilotSlotsByAirline = computed(() => {
    ...
    // Busiest airline first, then each airline's own flights in chronological order.
    const orderedAirlines = Object.entries(groups).sort(
        ([, a], [, b]) => b.length - a.length,
    );

    const ordered: Record<string, typeof props.pilotSlots> = {};

    for (const [airlineIcao, slots] of orderedAirlines) {
        ordered[airlineIcao] = [...slots].sort((a, b) =>
            operationalTime(a).localeCompare(operationalTime(b)),
        );
    }
...
<template>
    <div>
        ...
        <template v-else>
            ...
            <template v-else>
                <p class="mb-3 text-xs text-slate-400 dark:text-white/40">
                    {{ $t('EOBT') }}: {{ $t('Estimated Off-Block Time') }} ·
                    {{ $t('ETA') }}: {{ $t('Estimated Time of Arrival') }}
                </p>
